...
const loadConfiguration = () => {
  return new Promise((resolve, reject) => {
    ...
    fs.readFile("./config.json", "utf-8", (err, data) => {
      ...
      if (
        allConfiguration.configuration === undefined ||
        typeof allConfiguration.configuration !== "object"
      ) {
        return reject(
          new Error(
            "`configuration` object in `./config.json` missing or malformed.",
          ),
        );
      }
      ...
      const path = allConfiguration.configuration.path,
        ...
        baseURI = allConfiguration.configuration.baseURI;

      log("Checking base URI...");

      if (baseURI === undefined || typeof baseURI !== "string") {
        return reject(new Error("Base URI is malformed or missing."));
      }
      ...
      if (!fs.existsSync(path) || !fs.lstatSync(path).isDirectory()) {
        return reject(
          new Error(
            "Configured path " + path + "` does not exist or if not a folder.",
          ),
        );
      } else if (
        (path.slice(0, 2) != ".." && path[0] != "~" && path[0] != "/") ||
        path.trim() === "/"
      ) {
        return reject(
          new Error(
      ...
      if (shortcuts === undefined || !Array.isArray(shortcuts)) {
        return reject(
          new Error("Configured `shortcuts` array is malformed or missing"),
        );
      }
      ...
      for (var shortcutKey in shortcuts) {
        ...
        if (!(shortcut instanceof Object)) {
          return reject(
            new Error("Shortcut `" + String(shortcut) + "` is not an object."),
          );
        }
        const shortcutKeys = Object.keys(shortcut);
        if (
          !["shortcut", "url", "title"].every((key) =>
        ...
        if (
          ![sPath, sURL, sTitle].every((value) => typeof value === "string")
        ) {
          return reject(
            new Error(
              "Shortcut " +
...
const updateREADME = (shortcuts, version, baseURI) => {
  return new Promise((resolve, reject) => {
      ...
      const readmeFilledIn = readmeTemplate
        ...
        .replaceAll(
          ...
          shortcuts
            .map(
              (shortcut) =>
                `+ **${shortcut.title}**: ${baseURI}${shortcut.shortcut} --> ${shortcut.url}`,
            )
            .join("\n"),
        );
